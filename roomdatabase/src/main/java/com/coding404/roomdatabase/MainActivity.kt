...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateMember(id : Int) {
        ...
        val customDialog = Dialog(this) //다이어로그 객체
        val dialogBinding = CustomDialogBinding.inflate(layoutInflater) //xml
        customDialog.setContentView(dialogBinding.root) //다이어로그 객체에 레이아웃 연결
        ...
        dialogBinding.yesBtn.setOnClickListener {
            ...
            if(name.isNotEmpty() && email.isNotEmpty()) {
                ...
                lifecycleScope.launch {
                    ...
                    Toast.makeText(this@MainActivity, "정상 처리 되었습니다", Toast.LENGTH_SHORT).show()
                    customDialog.dismiss() //창 종료
                }
            } else {
                Toast.makeText(this@MainActivity, "값을 입력해 주세오", Toast.LENGTH_SHORT).show()
            }
    ...
    //삭제기능 - 리사이클러뷰의 생성자 매개변수를 넘기고, 리사이클러뷰에서는 아이디를 이 함수로 전달
    private fun deleteMember(id: Int) {

        //기본 알림창
        val dialog = AlertDialog.Builder(this)
        dialog.setTitle("삭제 하시겠습니까?")
        dialog.setPositiveButton("예") {dialog, _ -> //DialogInterface.OnClickListener 생략 가능
            //삭제 진행
            lifecycleScope.launch {
        ...
        dialog.create().show() //show까지 해서 보여지게함

//        lifecycleScope.launch {
//            // 해당 id로 멤버 정보를 가져온 후 삭제
//            val entity = repository.getMemberOne(id)
//            if (entity != null) {
//                repository.deleteMember(entity)
